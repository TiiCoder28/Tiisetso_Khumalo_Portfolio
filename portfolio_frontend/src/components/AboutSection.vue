...
<template>
  <section
    ref="sectionRef"
    id="about"
    class="relative w-full py-16 px-4 md:px-8 overflow-hidden"
  >
    <div class="relative z-10 max-w-7xl mx-auto justify-center flex flex-col items-center">
      ...
      <div class="flex flex-col gap-6 w-full max-w-5xl mx-auto">
        <div ref="terminalRef" class="relative group w-full">
        ...
        <div ref="highlightsRef" class="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          >
            ...
            <div class="relative text-center">
              ...
              <div class="text-2xl md:text-3xl lg:text-4xl font-bold bg-gradient-to-r from-cyan-400 to-purple-400 bg-clip-text text-transparent mb-1">{{ item.value }}</div>
              <div class="text-gray-400 text-xs md:text-sm uppercase tracking-wider leading-tight">{{ item.label }}</div>
            </div>
        ...
        <div class="p-6 md:p-8 bg-gradient-to-br from-slate-800/60 to-slate-900/60 backdrop-blur-sm rounded-xl border border-purple-500/20">
          ...
          <div class="grid md:grid-cols-2 gap-3 text-gray-300 text-base md:text-lg">
            <div class="flex items-center gap-3"><span class="w-2 h-2 bg-cyan-400 rounded-full flex-shrink-0"></span>Based in Johannesburg, South Africa</div>
            <div class="flex items-center gap-3"><span class="w-2 h-2 bg-cyan-400 rounded-full flex-shrink-0"></span>BSc Mathematics & Computer Science (UNISA) - In progress</div>
            <div class="flex items-center gap-3"><span class="w-2 h-2 bg-cyan-400 rounded-full flex-shrink-0"></span>Azure AI Engineer Associate Certified</div>
            <div class="flex items-center gap-3"><span class="w-2 h-2 bg-cyan-400 rounded-full flex-shrink-0"></span>Specializing in AI, RAG systems & LLM integrations</div>
          </div>
        </div>
      </div>
    </div>
